...
async def verify_password_function(hashed_password, plain_password) -> bool:

    salt_first = hashed_password[:16]
    salt_beyond = hashed_password[16:]

    transform = hashlib.pbkdf2_hmac('sha256', plain_password.encode('utf-8'), salt_first, 100_000)
